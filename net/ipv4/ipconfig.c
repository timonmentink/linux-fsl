...
static int __init ip_auto_config(void)
{
	...
	/* For Garz & Fricke systems kernel BOOTP/DHCP is only
	   allowed for root over nfs. 

	   The reason is that dhcp timeout my be too long when no dhcp server is available.
	   */
#ifdef CONFIG_ROOT_NFS
	if(!(ROOT_DEV ==  Root_NFS) )
	{
		printk("IP-Config: No root over NFS, skipping kernel network configuration...\n");
		ic_enable = 0;
		ic_myaddr = NONE;
		ic_servaddr = NONE;
		ic_gateway = NONE;
...
int ip_auto_config_setup(char *addrs)
{
	char *cp, *ip, *dp;
	int num = 0;
