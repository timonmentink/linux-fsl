...
static int imx_pcm_preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
	...
	pr_debug("%s Stream: %d, size %d, dev: %p buf-addr: %p\n", __func__, stream, size, pcm->card->dev, &buf->addr);
	...
	buf->area = dma_alloc_writecombine(pcm->card->dev, size,
					   &buf->addr, GFP_KERNEL);
	pr_debug("%s Stream: %d, size %d, dev: %p buf-addr: %p buf-area %p\n", __func__, stream, size, pcm->card->dev, &buf->addr, buf->area);
	if (!buf->area)
		return -ENOMEM;
	buf->bytes = size;
