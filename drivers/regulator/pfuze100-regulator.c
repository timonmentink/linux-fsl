...
extern void imx_anatop_set_ldo_bypass_enable(void);
				    ...
				    const struct i2c_device_id *id)
{
	...
	for (i = 0; i < regulator_num; i++) {
		...
		if(	   0 == of_property_read_string(config.of_node, "status", &status) 
			&& status 
			&& 0 == strcmp(status, "disabled"))
		{
			pr_debug("%s %s Regulator disabled\n", __func__, pfuze100_regulators[i].desc.name);
			continue;
		}
		...
		{
			int val = 0;
			if( !of_property_read_u32_index( config.of_node, "default-voltage", 0, &val))
			{
				struct regulator_dev * reg_dev = pfuze_chip->regulators[i];
				dev_dbg(&client->dev, "%s: %d default from dt: %d uV\n",__func__, i ,val);

				if( desc->ops->map_voltage)
					val = desc->ops->map_voltage(reg_dev, val - 10000, val + 10000);
				else
					val = regulator_map_voltage_iterate(reg_dev, val - 10000, val + 10000);
				if( val > 0 && desc->ops->set_voltage_sel)
					val = desc->ops->set_voltage_sel(reg_dev, val);

				if( desc->ops->enable)
					desc->ops->enable(reg_dev);
	...
	// While most SANTARO systems will be using pre-programmed PMICs,
	// which have all voltages configured properly from power-on,
	// we still have quite a few which power up with default voltages
	// that must be reconfigured for SANTARO.
	// These may need up to 400 to 500 us until all voltages are stable.
	udelay(500);

	// This is a little hack. Freescale does thes pmic setup and the ldo bypass in u-boot
	// To enable the bypass as early as possible after the configuration of the pmic supplies
	// We call this directly from here.
	imx_anatop_set_ldo_bypass_enable();
