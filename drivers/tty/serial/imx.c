...
#define UTS  uts_reg(sport) /* UART Test Register */
...
struct imx_port {
	...
	void (*rs485_tx_enable)(struct device * dev, int enable);
	void (*rs485_enable)(int enable);
	int rs485_tx_enable_gpio;
	struct regulator * rs485_xceiver;
};
...
static int imx_config_rs485(struct uart_port *port, struct serial_rs485 rs485conf)
{
	...
	if (!sport)
		return -ENOSYS;
	if (!sport->rs485_tx_enable)
		return -ENOSYS;

	spin_lock_irqsave(&sport->port.lock, flags);

	sport->rs485 = rs485conf;
	sport->rs485_tx_enable(sport->port.dev, USE_RS485_HALF_DUPLEX(sport) ? 0 : 1);
...
/*
 * Swich rs485 transceiver on or off
 */
static void rs485_transeiver_switch(struct imx_port *sport,  int on)
{
	int err;
	if(!sport) return;
	if(!sport->rs485_xceiver) return;

	if( on){
		if(( err= regulator_enable(sport->rs485_xceiver)))
			pr_err("%s: failed to enable regulator: %d\n", __func__, err);
	}else{
		regulator_disable(sport->rs485_xceiver);
	}
}

static void imx_stop_tx_half_duplex(struct imx_port *sport)
{
	unsigned long temp;

	pr_debug("%s %d: UCR4: 0x%08x\n",__func__, __LINE__, readl(sport->port.membase + UCR4) );
	WARN_ON(!(USE_IRDA(sport) || USE_RS485_HALF_DUPLEX(sport)));
	WARN_ON(!(readl(sport->port.membase + UCR4) & UCR4_TCEN));
	WARN_ON(!(readl(sport->port.membase + USR2) & USR2_TXDC));
	/*
...
pr_debug("%s %d\n", __func__, __LINE__);
	...
	if (USE_RS485_HALF_DUPLEX(sport) && sport->rs485_tx_enable)
	{
		pr_debug("%s %d tx disable\n",__func__, __LINE__);
		sport->rs485_tx_enable(sport->port.dev, 0);
	}
pr_debug("%s %d\n", __func__, __LINE__);
...
static void dma_tx_work(struct work_struct *w)
{
	...
	if (sport->tx_bytes > 0) {
		...
		desc = chan->device->device_prep_slave_sg(chan, sgl,
				sport->dma_tx_nents, DMA_TO_DEVICE, 0, 0);
		if (!desc) {
...
static void imx_start_tx(struct uart_port *port)
{
	...
	if (USE_RS485_HALF_DUPLEX(sport) && sport->rs485_tx_enable)
		sport->rs485_tx_enable(sport->port.dev, 1);
...
{
	struct imx_port *sport = dev_id;
	unsigned int sts, ucr1;

	pr_debug("%s %d: USR1 0x%08x  USR2 0x%08x UCR1: 0x%08x UCR4: 0x%08x\n",__func__, __LINE__,  
		readl(sport->port.membase + USR1), readl(sport->port.membase + USR2),
		readl(sport->port.membase + UCR1), readl(sport->port.membase + UCR4)
		);

	sts = readl(sport->port.membase + USR1);
	ucr1 = readl(sport->port.membase + UCR1);

	if ((sts & USR1_RRDY) && (ucr1 & UCR1_RRDYEN)) {
		if (sport->enable_dma)
			imx_dma_rxint(sport);
		else
...
#define TXTL 16 /* reset RS485 needs high threshold */
...
static int imx_uart_dma_init(struct imx_port *sport)
{
	...
	sport->dma_data.dma_request0 = pdata->dma_req_rx;
	...
	/* prepare for TX : */
	sport->dma_data.dma_request0 = pdata->dma_req_tx;
...
static int imx_startup(struct uart_port *port)
{
	...
	if (USE_RS485(sport)) {
		rs485_transeiver_switch(sport, 1);
	}
	...
	// Clear TCEN, may be enabled from last run
	temp = readl(sport->port.membase + UCR1);
	temp &= ~(UCR1_TRDYEN);
	writel(temp, sport->port.membase + UCR1);
	temp = readl(sport->port.membase + UCR4);
...
static void imx_shutdown(struct uart_port *port)
{
	...
	if (USE_RS485(sport)) {
		rs485_transeiver_switch(sport, 0);
	}
	console_unlock();
}
			   ...
			   int *parity, int *bits)
{
	...
	if (readl(sport->port.membase + UCR1) & UCR1_UARTEN) {
		...
		*parity = 'n';
		...
		if (*baud != baud_raw)
			dev_warn(sport->port.dev, "Console IMX rounded baud rate from %d to %d\n",
				baud_raw, *baud);
	}
}
...
#ifdef CONFIG_OF
static void serial_imx_rs485_tx_enable(struct device *dev, int enable)
{
	struct platform_device * pdev = to_platform_device(dev);
	struct imx_port *sport;
	if(!pdev)	return;

	sport = platform_get_drvdata(pdev);
	gpio_set_value_cansleep(sport->rs485_tx_enable_gpio, enable);
}
		...
		struct platform_device *pdev)
{
	...
	if (of_get_property(np, "rs485-mode", NULL))
	{

		sport->rs485_tx_enable_gpio = of_get_named_gpio(np, "rs485-tx-enable-gpio", 0);

		if( sport->rs485_tx_enable_gpio < 0)
		{
			ret = sport->rs485_tx_enable_gpio;
			dev_err( &pdev->dev, " failed to get tx_enable gpio: %d\n", ret);
			sport->rs485_tx_enable_gpio = 0;
			return ret;
		}

		ret = gpio_request( sport->rs485_tx_enable_gpio, "rs485 tx enable");
		if(ret)
		{
			dev_err( &pdev->dev, " failed to request tx_enable gpio: %d\n", ret);
			sport->rs485_tx_enable_gpio = 0;
			return ret;
		}
		ret = gpio_direction_output(sport->rs485_tx_enable_gpio, 0);
		if(ret)
		{
			dev_err( &pdev->dev, " failed to change direction of tx_enable gpio: %d\n", ret);
			sport->rs485_tx_enable_gpio = 0;
			return ret;
		}

		sport->rs485.flags = SER_RS485_ENABLED;
		if (of_get_property(np, "rs485-mode-half-duplex", NULL))
			sport->rs485.flags |= SER_RS485_RTS_ON_SEND;

		sport->rs485_tx_enable = serial_imx_rs485_tx_enable;

		sport->rs485_xceiver = devm_regulator_get(&pdev->dev, "rs485");
		if (IS_ERR(sport->rs485_xceiver))
		{
			dev_warn(&pdev->dev, "couldn't find regulator for transeiver: %ld",  PTR_ERR(sport->rs485_xceiver));
			sport->rs485_xceiver = NULL;
		}
		else{
			dev_info(&pdev->dev, "got regulator for transeiver\n");
		}
	}
		...
		struct platform_device *pdev)
{
	...
	if (pdata->flags & IMXUART_USE_DCEDTE)
		sport->use_dcedte = 1;


	if (pdata->flags & IMXUART_RS485)
		sport->rs485.flags = SER_RS485_ENABLED;

	if (pdata->flags & IMXUART_RS485_HALFD)
		sport->rs485.flags |= SER_RS485_RTS_ON_SEND;

	sport->rs485_tx_enable = pdata->rs485_tx_enable;
	sport->rs485_enable = pdata->rs485_enable;
}
...
static int serial_imx_probe(struct platform_device *pdev)
{
	...
	if (ret < 0)
	{
		dev_err(&pdev->dev," Failed to parse device tree setings\n");
		return ret;
	}
...
#ifdef DEBUG
	pr_debug("%s: got clock: %s\n",__func__, __clk_get_name(sport->clk_ipg));
#endif
...
#ifdef DEBUG
	pr_debug("%s: got clock: %s\n",__func__, __clk_get_name(sport->clk_per));
#endif
