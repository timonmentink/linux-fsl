...
static int gpiod_export(struct gpio_desc *desc, bool direction_may_change)
{
	...
	pr_debug("gpiod_export: Names: %p, %s, Offset: %d, Name %s\n",
		desc->chip->names, 
		desc->chip->names[0]?desc->chip->names[0]:"Null",
	  	offset, ioname);
...
int gpio_request_one(unsigned gpio, unsigned long flags, const char *label)
{
	...
	if (flags & GPIOF_ACT_LOW)
		err = gpio_sysfs_set_active_low(gpio, (flags & GPIOF_ACT_LOW) ? 1 : 0);
...
static void gpiod_set_value_cansleep(struct gpio_desc *desc, int value)
{
	...
	if (!desc)
		return;
	chip = desc->chip;
	pr_debug("%s: set %s to %d: flags: 0x%08lx set: 0x%08x\n", __func__, chip->label, value, desc->flags, (unsigned int ) chip->set);
	trace_gpio_value(desc_to_gpio(desc), 0, value);
	if (test_bit(FLAG_OPEN_DRAIN,  &desc->flags))
		_gpio_set_open_drain_value(desc, value);
	else if (test_bit(FLAG_OPEN_SOURCE,  &desc->flags))
