...
#define AUO_PIXCIR_REG_VERSION          0x40
...
#define AUO_PIXCIR_REPORT_POINTS	5
...
#define DBG_RAW		0x00001000
static unsigned int DbgLevel = DBG_MODULE|DBG_SUSP;


#define PIXCIR_DBG(level, fmt, args...)  do{ if( (level&DbgLevel)>0 ) \
					printk("[pixcir]: " fmt, ## args); }while(0)
				   ...
				   struct auo_point_t *point)
{
	...
	const struct auo_pixcir_ts_platdata *pdata = ts->pdata;
        ...
        for (i = 0; i < AUO_PIXCIR_REPORT_POINTS; i++) {
                point[i].coord_x =
                        raw_coord[5 * i + 3] << 8 | raw_coord[5 * i + 2];
                point[i].coord_y =
                        raw_coord[5 * i + 5] << 8 | raw_coord[5 * i + 4];
                point[i].id =
                        raw_coord[5 * i + 6];
                ...
                if (point[i].coord_x > pdata->x_max ||
					...
					PIXCIR_DBG(DBG_POINT, "coord[%d]: (%d , %d).\n", point[i].id, point[i].coord_x, point[i].coord_y );
				}

        }
		if( DBG_RAW & DbgLevel){
			for(i = 0; i< 27; i++)
				printk( "%02X", raw_coord[i] );
			for(i = 0; i< AUO_PIXCIR_REPORT_POINTS; i++)
				printk( "  %04d %04d", point[i].coord_x, point[i].coord_y );
			printk( "\n");
		}
...
static struct auo_pixcir_ts_platdata *auo_pixcir_parse_dt(struct device *dev)
{
	...
	if (of_property_read_u32(np, "int-setting", &pdata->int_setting)) {
		dev_err(dev, "failed to get int_setting property\n");
		return ERR_PTR(-EINVAL);
	}
			    ...
			    const struct i2c_device_id *id)
{
	...
	error = i2c_smbus_read_i2c_block_data(client, AUO_PIXCIR_REG_VERSION,
	...
	if (error)
	{
		dev_err(&client->dev, "auo_pixcir_int_config failed, %d\n",
			error);
		return error;
	}
	...
	if (error)
	{
		dev_err(&client->dev, "auo_pixcir_stop failed, %d\n",
			error);
		return error;
	}
