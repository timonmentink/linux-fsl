...
#define WM9705_MAX_ADC_VAL  0xFFFF 
...
static int filter_samples(int *samples)
{
    ...
    for (i = 0; i < NUMBER_SAMPLES_PER_POINT; i++)
        if (samples[i] != WM9705_MAX_ADC_VAL) {
            n_valid_samples++;
            mean += samples[i];
        }
    ...
    if (max_dist > (MAX_DISTANCE * MAX_DISTANCE)) {
        samples[candidate_for_discard] = WM9705_MAX_ADC_VAL;
...
static int wm9705_poll_sample(struct wm97xx *wm, int adcsel, int *sample)
{
	unsigned int i = 0;
	int val = 0;
	int adc_values[NUMBER_SAMPLES_PER_POINT];

	/* set up digitiser */
	if (adcsel & 0x8000)
		adcsel = ((adcsel & 0x7fff) + 3) << 12;

	/* set controller to continously drive the planes and keep sampling */
	wm97xx_reg_write(wm, AC97_WM97XX_DIGITISER1,
			 adcsel | WM97XX_CTC | WM97XX_CM_RATE_48K);

	/* wait for planes to settle down */
	udelay(AC97_LINK_FRAME * (1 << delay));

	for(i = 0; i < NUMBER_SAMPLES_PER_POINT; i++) {
		adc_values[i] = WM9705_MAX_ADC_VAL;
		val = wm97xx_reg_read(wm, AC97_WM97XX_DIGITISER_RD);
		if(!((val & WM97XX_PEN_DOWN) == WM97XX_PEN_DOWN)) {
			wm97xx_reg_write(wm, AC97_WM97XX_DIGITISER1, 0);
			return RC_PENUP;
		}
		adc_values[i] = val & 0xfff;
	}

	wm97xx_reg_write(wm, AC97_WM97XX_DIGITISER1, 0);
	
	*sample = filter_samples(adc_values);
	if(*sample == TOUCH_SAMPLE_IGNORE)
		return RC_AGAIN;
