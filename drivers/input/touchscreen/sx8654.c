...
/* bits for RegTouch1 */
#define CONDIRQ				0x20
#define TSTYPE_5W			0x10
/* FIXME: 7*RTOTX+RTOTY (datasheet sx8765), 28k is much more
          than that value for our 10.1" an 12.1"
		  touch, so it's save to use the lowest
		  to decrease sensitivity to prevent
		  phantom touches on very slight touch
          actuations */
#define RPNDT_28K			0x0c
...
/* This is the maxiumum for two coordinates within 5ms */
#define POWDLY_2_27MS			0x0c
...
static void __devinit sx8654_ts_reset(struct i2c_client *client,
						struct sx8654 *sx8654)
{
	/* Datasheet: low pulse min. 1 us, p. 11 */
	gpio_set_value(sx8654->reset_gpio, 0);
	udelay(5);
	gpio_set_value(sx8654->reset_gpio, 1);
}
...
static int __devinit sx8654_probe_dt(struct i2c_client *client,
						struct sx8654 *sx8654)
{
	int error;
	...
	of_id = of_match_device(sx8654_of_match, &client->dev);
	dev_dbg(&client->dev, "%s np: 0x%08x, of_id: 0x%08x\n", __func__,
				(unsigned int)np, (unsigned int)of_id);

	sx8654->reset_gpio = of_get_named_gpio( np, "reset-gpio", 0);
	if(sx8654->reset_gpio < 0) {
	...
	/* FIXME: Datasheet: POR time 1ms, p. 11, we assume here that 1ms is always
	   elapsed instead adding an addistional msleep */
	error = devm_gpio_request_one(&client->dev, sx8654->reset_gpio,
				GPIOF_OUT_INIT_LOW, "sx8654 reset");
	if (error < 0) {
			...
			const struct i2c_device_id *id)
{
...
#ifdef CONFIG_OF
	error = sx8654_probe_dt(client, sx8654);
	if( error < 0)
	{
	...
	error = i2c_smbus_write_byte_data(client, I2C_REG_TOUCH1,
					  CONDIRQ | TSTYPE_5W | RPNDT_28K | FILT_7SA);
	if (error) {
