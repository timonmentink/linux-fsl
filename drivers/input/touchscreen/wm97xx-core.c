...
static int wm97xx_read_samples(struct wm97xx *wm)
{
	...
	if (rc & RC_PENUP) {
		if (wm->pen_up_count >= TS_MIN_TOUCHUP) {
			wm->pen_is_down = 0;
		...
		if (!(rc & RC_AGAIN)) {
			...
			*/
			wm->ts_reader_interval = msecs_to_jiffies(5);
		}
	} else if (rc & RC_VALID) {
#ifdef MEASURE_SAMPLE_TIME
		static int toggle = 0;
		...
		if(!toggle) {
			toggle = 1;
			gpio_set_value(KP_COL0, 1);
		} else {
			toggle = 0;
			gpio_set_value(KP_COL0, 0);
		}
#endif
		wm->pen_up_count = 0;
		dev_dbg(wm->dev,
...
static int wm97xx_probe(struct device *dev)
{
	...
	if( !pdata->enable_ts)
	{
		dev_info(dev, "touch functionality disabled\n");
		return -ENODEV;
	}

#ifdef MEASURE_SAMPLE_TIME
	gpio_request(KP_COL0, "GPIO");
	gpio_direction_output(KP_COL0, 0);
#endif

	wm = kzalloc(sizeof(struct wm97xx), GFP_KERNEL);
	if (!wm)
	{
		dev_err(dev, "Failed to allocate driver memory\n");
		return -ENOMEM;
	}
	...
	if(pdata->mach_ops) {
		wm->pen_irq = pdata->irq;
		irq_set_irq_type(wm->pen_irq, IRQ_TYPE_EDGE_BOTH);
		wm97xx_register_mach_ops(wm, pdata->mach_ops);
	}
