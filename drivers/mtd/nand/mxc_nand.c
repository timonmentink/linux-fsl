...
static const char const *part_probes[] = {
	"GufRedBoot", "RedBoot", "cmdlinepart", NULL };
...
static void wait_op_done(struct mxc_nand_host *host, int useirq)
{
	...
	if (useirq) {
		...
		if (host->show_error && (max_retries < 0))
			printk("%s: timeout INT not set\n",
			      __func__);
	}
}
...
static uint16_t get_dev_status_v3(struct mxc_nand_host *host)
{
	...
	ret = readl(NFC_V3_CONFIG1) >> 16;
	if (host->show_error && (ret & 0x1))
		printk("MXC_NAND: status error 0x%04x\n", ret);
	return ret;
}
				 ...
				 u_char *read_ecc, u_char *calc_ecc)
{
	...
	if (((ecc_status & 0x3) == 2) || ((ecc_status >> 2) == 2)) {
		if (host->show_error)
			printk("MXC_NAND: HWECC uncorrectable 2-bit ECC error (0x%04x)\n", ecc_status);
				 ...
				 u_char *read_ecc, u_char *calc_ecc)
{
	...
	do {
		...
		if (err > err_limit) {
			if (host->show_error)
				printk("UnCorrectable RS-ECC Error 0x%08x\n", ecc_stat);
			return -1;
		} else {
			ret += err;
		}
