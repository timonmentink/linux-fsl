...
static int __init imx2_wdt_probe(struct platform_device *pdev)
{
	...
	if( np)
	{		
		of_property_read_u32(np, "fsl,default-timeout", &timeout);
		of_property_read_u32(np, "fsl,reset-wdog-b", &imx2_wdt.trigger_wdog_b);
		pr_debug("%s %p Timeout: %d  External WDOG: %d\n", __func__, np, timeout, imx2_wdt.trigger_wdog_b );
	}
	...
	reset = __raw_readw(imx2_wdt.base + IMX2_WDT_WRSR);

	dev_info(&pdev->dev,
		"IMX2+ Watchdog Timer enabled. timeout=%ds (nowayout=%d) last reset: 0x%04X\n",
						imx2_wdt.timeout, nowayout, reset);
	return 0;
