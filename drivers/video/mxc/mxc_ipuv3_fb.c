...
static int mxcfb_set_fix(struct fb_info *info)
{
	...
	dev_dbg(info->device, "%s vxres: %d, bpp %d\n", __func__,  var->xres_virtual, var->bits_per_pixel );
...
static bool mxcfb_need_to_clear_screen(struct fb_info *fbi)
{
	...
	/*
	 * Ignore activate, already checked in mxcfb_need_to_set_par().
	 */
	mxc_fbi->cur_var.activate = fbi->var.activate;
#ifdef DEBUG
	dump_fb_var_changes( &mxc_fbi->cur_var, &fbi->var, __func__);
#endif
...
static bool mxcfb_need_to_set_par(struct fb_info *fbi)
{
	...
	return mxcfb_need_to_clear_screen(fbi);
}
...
static int mxcfb_set_par(struct fb_info *fbi)
{
	...
	if (mxcfb_need_to_clear_screen(fbi)) {
		/*
		 * Clear the screen if framebuffer is reconfigured.
		 */
	dev_dbg(fbi->device, "Clear Screen\n");
	...
	if (!mxc_fbi->overlay) {
		...
		dev_dbg(fbi->device, "pixclock = %ul Hz fb sync: 0x%08x"
			" interlaced: %d, odd 1st %d, int clk %d, Hsync pol %d,"
		    " Vsync pol %d, clk pol %d, data pol %d, en pol %d, clk idle %d	\n",
			(u32) (PICOS2KHZ(fbi->var.pixclock) * 1000UL), 
				fbi->var.sync,
				sig_cfg.interlaced,
		...
		dev_dbg(fbi->device, "%s %d ipu parameter"
			" pixclk %d, x %d, y %d, fmt 0x%08x, "
			" left %d, hsync %d, right %d, "
			" upper %d, vsync %d, lower %d "
			"\n",
					__func__, __LINE__,
					(int)((PICOS2KHZ(fbi->var.pixclock)) * 1000UL),
					(int)fbi->var.xres, (int)fbi->var.yres,
					(int)out_pixel_fmt,
					(int)fbi->var.left_margin,
					(int)fbi->var.hsync_len,
					(int)fbi->var.right_margin,
					(int)fbi->var.upper_margin,
					(int)fbi->var.vsync_len,
					(int)fbi->var.lower_margin
			);
...
mxcfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	dev_dbg(info->device, "%s: X off: %d, y off: %d \n", __func__, var->xoffset, var->yoffset);
...
static int mxcfb_mmap(struct fb_info *fbi, struct vm_area_struct *vma)
{
	...
	dev_dbg(fbi->device, "%s smem: %u offset: %lu\n", __func__, fbi->fix.smem_len, offset);
...
static int mxcfb_map_video_memory(struct fb_info *fbi)
{
	pr_debug("%s smemlen: %d, vy %d , line %d\n", __func__, fbi->fix.smem_len, fbi->var.yres_virtual, fbi->fix.line_length );
...
static int mxcfb_option_setup(struct platform_device *pdev, struct fb_info *fbi)
{
	...
	if (!options || !*options)
	{
		pr_info("%s no options from command line\n", __func__);
		return 0;
	}
				...
				struct ipuv3_fb_platform_data *plat_data)
{
	...
	err = of_property_read_u32(np, "rotate", &rotate);
	if (err) {
		dev_dbg(&pdev->dev, "get of property 'rotate' fail: %d\n", err);
		rotate = 0;
		err = 0;
	}
	...
	else if (!strncmp(pixfmt, "YUV422P", 6))
				plat_data->interface_pix_fmt = IPU_PIX_FMT_YUV422P;
	else {
...
static int mxcfb_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(mxcfbi->ipu)) {
		ret = -ENODEV;
		dev_warn(&pdev->dev, "failed to get IPU %d: 0x%p",mxcfbi->ipu_id, mxcfbi->ipu);
		goto get_ipu_failed;
	}
