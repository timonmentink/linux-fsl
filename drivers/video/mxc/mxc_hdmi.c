...
static void mxc_hdmi_phy_init(struct mxc_hdmi *hdmi)
{
	...
	{
		dev_dbg(&hdmi->pdev->dev, "%s: stop phy init: Cable: %d, fb reg 0x%x, Blank: %d \n", __func__, hdmi->cable_plugin, hdmi->fb_reg, hdmi->blank);
		return;
	}
...
static int mxc_hdmi_read_edid(struct mxc_hdmi *hdmi)
{
	...
	if (!hdcp_init)
	{
		dev_dbg(&hdmi->pdev->dev, "%s Read edid using i2c\n", __func__);
		ret = mxc_edid_read(hdmi_i2c->adapter, hdmi_i2c->addr,
				hdmi->edid, &hdmi->edid_cfg, hdmi->fbi);
	}
	else {
		dev_dbg(&hdmi->pdev->dev, "%s Read edid internally\n", __func__);
					...
					unsigned long val, void *v)
{
	...
	case FB_EVENT_BLANK:
		pr_debug("%s %d BLANK data: 0x%p, hdmi->blank: 0x%x \n",__func__, __LINE__, event->data, (unsigned int) hdmi->blank );
			      ...
			      struct mxc_dispdrv_setting *setting)
{
	...
	if (irq < 0)
	{
		dev_err(&hdmi->pdev->dev, "Error: irq < 0!\n");
		return -ENODEV;
	}
	...
	/*
	dev_dbg(&hdmi->pdev->dev,"%s %d: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n", __func__, __LINE__, 
		hdmi_readb(HDMI_IH_PHY_STAT0),
		hdmi_readb(HDMI_PHY_STAT0),
		hdmi_readb(HDMI_PHY_INT0),
		hdmi_readb(HDMI_PHY_MASK0),
		hdmi_readb(HDMI_PHY_POL0)
	);
	*/
	...
	dev_dbg(&hdmi->pdev->dev, "%s exit: %d\n", __func__, ret);
...
static int mxc_hdmi_probe(struct platform_device *pdev)
{
	...
	{
		dev_err(&pdev->dev, "Missing hdmi_i2c or hdcp_init\n");
		return -ENODEV;
	}
	...
	if (IS_ERR(hdmi_class)) {
		dev_err(&pdev->dev, "Failed to create hdmi_class\n");
	...
	if (IS_ERR(temp_class)) {
		dev_err(&pdev->dev, "Failed to create temp_class\n");
	...
	dev_dbg(&pdev->dev, "HDMI probed successful\n");
	return 0;
edispdrv:
