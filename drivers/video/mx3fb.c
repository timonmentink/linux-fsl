...
static void mx3fb_enable_display(struct fb_info *fbi) {
	...
	if(lcd_dev) {
		lcd_set_power_dev(lcd_dev, FB_BLANK_UNBLANK);
	}
...
static void mx3fb_disable_display(struct fb_info *fbi) {
	...
	if(lcd_dev) {
		lcd_set_power_dev(lcd_dev, FB_BLANK_POWERDOWN);
	}
}
...
static int mx3fb_probe(struct platform_device *pdev)
{
	...
	if (ret < 0)
		goto eisdc0;
	if( mx3fb_pdata && mx3fb_pdata->fb_initialized)
		mx3fb_pdata->fb_initialized(mx3fb->fbi);
