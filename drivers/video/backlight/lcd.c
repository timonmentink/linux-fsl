				 ...
				 unsigned long event, void *data)
{
	...
	pr_debug("fb notifier callback: Event %d\n",(unsigned int) event);
...
int lcd_set_power_dev(struct device *dev, unsigned int power) 
{
	int ret = 0;
	...
	pr_debug("lcd_device: %s, device: 0x%08X, lcd_device: 0x%08X\n", __func__, (unsigned int)dev, (unsigned int)ld);
		...
		void *devdata, struct lcd_ops *ops)
{
	...
	pr_debug("lcd_device: parent: 0x%08X, lcd_device: 0x%08x\n", (unsigned int)parent, (unsigned int)new_ld);
