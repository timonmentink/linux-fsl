...
static int ci_hdrc_imx_probe(struct platform_device *pdev)
{
	...
	gpio_vbus = of_get_named_gpio(np, "gpio-vbus", 0);
	if(gpio_vbus >= 0)
	{	
		ret = gpio_request(gpio_vbus,"USB Power");
		if(ret)
		{
			dev_err(&pdev->dev, "%s: failed to request gpio-vbus pin: %d\n",__func__, ret);
		}else{
			ret = gpio_direction_output(gpio_vbus,0);
		}
	}
...
static int imx_controller_suspend(struct device *dev)
{
	...
	if (data->usbmisc_data && 
		(data->usbmisc_data->index == 2 || data->usbmisc_data->index == 3) )
   	{
		// The HSIC Standby seems not to work
		return 0;
	}
