...
static int usb3503_switch_mode(struct usb3503 *hub, enum usb3503_mode mode)
{
	...
	case USB3503_MODE_HUB:
		...
		val = usb3503_read_register(i2c, USB3503_VIDL);
		if( val < 0)
		{
			dev_warn(&i2c->dev, "Could not read register: %d\n", val);
			goto err_hubmode;
		}
		vid = val;
		val = usb3503_read_register(i2c, USB3503_VIDM);
		if( val < 0)
		{
			dev_warn(&i2c->dev, "Could not read register: %d\n", val);
			goto err_hubmode;
		...
		val = usb3503_read_register(i2c, USB3503_PIDL);
		if( val < 0)
		{
			dev_warn(&i2c->dev, "Could not read register: %d\n", val);
			goto err_hubmode;
		...
		pid = val;
		val = usb3503_read_register(i2c, USB3503_PIDM);
		if( val < 0)
		{
			dev_warn(&i2c->dev, "Could not read register: %d\n", val);
			goto err_hubmode;
		...
		pid |= val << 8;
		dev_info(&i2c->dev, "Found Hub %04x:%04x\n", vid, pid);
#ifdef DEBUG_USB3503
		{
			int i;
			for(i = 0; i < 255; i++)
				dev_dbg(&i2c->dev, "0x%02x: 0x%02x\n",i,usb3503_read_register(i2c, i));
		}
#endif
		...
		if (err < 0) {
			dev_err(&i2c->dev, "SP_ILOCK failed, set connect (%d)\n", err);
			goto err_hubmode;
		}


		//\FIXME add of configuration to select the enabled ports 
...
static int usb3503_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	hub->refclk = clk_get(&i2c->dev, NULL);
	if (IS_ERR(hub->refclk)) {
			goto err_out;
	} else {
		int clk_frequency = clk_get_rate(hub->refclk);
		clk_prepare_enable(hub->refclk);
		dev_dbg	( &i2c->dev, "%s got clk with rate: %d\n", __func__, clk_frequency);
	}
	...
	msleep(2); // Some time for the reset
...
err_out:
	if(err)
		dev_err(&i2c->dev, "Failed to initialize hub: %d\n", err);
	kfree(hub);
