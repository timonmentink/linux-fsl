...
/* Sets the bypass mode, in the freescale setup this is done by uboot */
void imx_anatop_ldo_bypass_enable(void)
{
	ldo_bypass = 1;
}

/* This actually sets the bypass enable.
   Make sure the pmic is already setup to deliver the correct input voltages!
*/
void imx_anatop_set_ldo_bypass_enable(void)
{
	u32 val;
	if( !ldo_bypass)
		return;

	pr_info("Actually switch to ldo-bypass mode\n");
	regmap_read(anatop, ANADIG_REG_CORE, &val);
	val = val 
		| (ANADIG_REG_TARG_MASK << ANADIG_REG2_TARG_SHIFT)	// VDDSOC
		|  ANADIG_REG_TARG_MASK;							// VDDARM

	regmap_write(anatop, ANADIG_REG_CORE, val);
	udelay(70);	/* bypass need 70us to be stable ( This comes from PU bypass, not sure what times are valid for SOC and ARM*/
