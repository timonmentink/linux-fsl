...
#define IMX2_WDT_WCR_SRS	(1 << 4)	/* -> Watchdog SRS */
#define IMX2_WDT_WCR_WDA	(1 << 5)	/* -> Watchdog WDOG_B */

static void __iomem *wdog_base;
static struct clk *wdog_clk;
static u32 wdog_source = 1; /* use WDOG1 default */
static unsigned int wcr_enable = IMX2_WDT_WCR_WDE;
...
void __init mxc_arch_reset_init_dt(void)
{
	...
	if (np){		
		...
		of_property_read_u32(np, "fsl,wdog-reset-srs", &srs);
		of_property_read_u32(np, "fsl,wdog-reset-wdog-b", &wdog_b);

		wcr_enable =  IMX2_WDT_WCR_WDE
					| (srs ? 0 : IMX2_WDT_WCR_SRS)
					| (wdog_b ? 0 : IMX2_WDT_WCR_WDA);
	}
	// Hard override of the settings from dt as this wdog works different
	if (cpu_is_mx1()) wcr_enable = (1 << 0);

	pr_info("Use WDOG%d as reset source with 0x%04x as trigger\n", wdog_source, wcr_enable);
