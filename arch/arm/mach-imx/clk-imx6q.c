...
extern void fixup_bootloader_settings(void );
...
static int cko_debug_sel=-1;
static int cko_debug_src=-1;
#define ARRAY_LENGTH(array) (sizeof((array))/sizeof((array)[0]))
static int __init __parse_debug_cko(char *options)
{
	int i;
	for( i = 0; i < ARRAY_LENGTH(cko2_sels); i++)
	{
		if( !strcmp(options, cko2_sels[i]))
		{
			cko_debug_sel = i;
			cko_debug_src = 2;
			return 0;
	...
	for( i = 0; i < ARRAY_LENGTH(cko1_sels); i++)
	{
		if( !strcmp(options, cko1_sels[i]))
		{
			cko_debug_sel = i;
			cko_debug_src = 1;
			return 0;
	...
	pr_err("Option 'debugcko' cannot be set to %s\n Select one of:", options);
	for( i = 0; i < ARRAY_LENGTH(cko2_sels); i++)
		pr_err("%s\n", cko2_sels[i]);
	for( i = 0; i < ARRAY_LENGTH(cko1_sels); i++)
		pr_err("%s\n", cko1_sels[i]);
...
__setup("debugcko=", __parse_debug_cko);
...
static void dump_ccm( void __iomem *base, int len)
{
#ifdef DEBUG
	int i;
	void __iomem * addr;
...
static void __init imx6q_clocks_init(struct device_node *ccm_node)
{
	...
	early_pr_dbg("%s: anatop: 0x%08X, %s\n",__func__, base, np->name);
	WARN_ON(!base);

	dump_ccm(anatop_base, 0x170);
	...
	early_pr_dbg("%s: 11: is 6q :%s\n",__func__, cpu_is_imx6q()?"true":"false");
	...
	//init_ldb_clks(pll5_video_div);  /* Its possible to use PLL5, but this is not effected by the spread spectrum settings, so we go back to pll2 */
	...
	if(cko_debug_sel != -1 && (cko_debug_src == 1 || cko_debug_src == 2 ))
	{
		pr_info("Enable debug clock out: %d %d %s\n", cko_debug_src, cko_debug_sel, 
			cko_debug_src==1? cko1_sels[cko_debug_sel]:cko2_sels[cko_debug_sel]);	
		ret = -1;

		if( cko_debug_src == 1)
		{
			struct  clk *c = __clk_lookup(cko1_sels[cko_debug_sel]);
			if(!c)
				pr_warn("failed to find %s\n", cko1_sels[cko_debug_sel]);
			else{
				ret = clk_set_parent(clk[cko1_sel], c);
				if (ret) pr_warn("failed to set  cko: %d\n", ret);
				else
					ret = clk_set_parent(clk[cko], clk[cko1]);
			}
		}else{
			struct  clk *c = __clk_lookup(cko2_sels[cko_debug_sel]);
			if(!c)
				pr_warn("failed to find %s\n", cko2_sels[cko_debug_sel]);
			else{
				ret = clk_set_parent(clk[cko2_sel], c);
				if (ret) pr_warn("failed to set  cko: %d\n", ret);
				else
		...
		if(!ret)
		{
			ret = clk_prepare_enable(clk[cko]);
			if (ret) pr_err("failed to enable  cko: %d\n", ret);
		}
		ret = readl(ccm_base + 0x60);
		pr_info("%s: CCM_COSRC 0x%08x\n", __func__,  ret);
	}else{
		/*
		...
		/* Set the clock to 12MHz as this is what we us in Santaro, 
		   \FIXME not nice to hard code it here
		   */
		rate = clk_round_rate(clk[cko2], 12000000);
		ret = clk_set_rate( clk[cko2], rate);
		if(ret)
			pr_err("%s: Failed to set clko2 to %d, ret: %d\n", __func__, rate, ret);
		else{
			pr_info("%s: set clko2 to %d, ret: %d\n", __func__, rate, ret);
			ret = clk_set_parent(clk[cko], clk[cko2]);
			if (ret) pr_warn("failed to set  cko: %d\n", ret);
		}
	}
	...
	/* Workarround for PCIe not coming up on sdc-cspu platform
	   'phy link never came up'
	   */
	{
		bool pci_enabled = false;
		const char * compatible = "fsl,imx6q-pcie";
		struct device_node *np;
		struct property *pp;
		np = of_find_compatible_node(NULL, NULL, compatible);
		if (np){
			pp = of_find_property(np, "status", NULL);
			if(pp && pp->value)
				if(strncmp(pp->value, "disabled", pp->length))
					pci_enabled = true;
			of_node_put(np);
		}
		...
		if(pci_enabled)
		{
			pr_info("PCI enbaled, starting clock...\n");
